...
<body id="top">
    ...
    <main>
            ...
            <div class="container">
                <div class="row justify-content-center align-items-center">
                    ...
                    <div class="col-lg-5 col-12 mb-5">
                        ...
                        <h2 class="text-white">Edit<br>Anggota KK</h2>
        ...
        <form method="post" action="{{ route('anggotakks.update', $anggotakks->id) }}">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="kk_id">ID KK:</label>
            <select class="form-control" id="kk_id" name="kk_id" required>
                @foreach($kksList as $kk)
                    <option value="{{ $kk->id }}" {{ $anggotakks->kk_id == $kk->id ? 'selected' : '' }}>{{ $kk->id }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="penduduk_id" name="penduduk_id" required>
                @foreach($penduduksList as $penduduk)
                    <option value="{{ $penduduk->id }}" {{ $anggotakks->penduduk_id == $penduduk->id ? 'selected' : '' }}>{{ $penduduk->id }}</option>
                @endforeach
            </select>
        </div>

        <div class="form-group">
            <label for="hubungankk_id">ID Hubungan:</label>
            <select class="form-control" id="hubungankk_id" name="hubungankk_id" required>
                @foreach($hubungankksList as $hubungankk)
                    <option value="{{ $hubungankk->id }}" {{ $anggotakks->hubungankk_id == $hubungankk->id ? 'selected' : '' }}>{{ $hubungankk->id }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
